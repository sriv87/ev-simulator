...
def get_arrival_time(num_vehicles):
    if use_case == 1: # Residential use case
        arrival_time = np.random.normal(20,1,num_vehicles)
        return arrival_time
...
def get_starting_SoC(is_slow,num_vehicles):
    if is_slow == 1: # Slow charge: one charge per day
        dist_traveled = np.random.lognormal(4.67017251945698, 0.532230403897875, num_vehicles)
    ...
    for it in iterator:
        ...
        for veh in range(num_vehicles):
            ...
            if end_time[veh] > start_time[veh]:
                ...
                iterator_time = np.arange(start_time[veh],max_time_interval,1)
                for time in iterator_time:
                    e_load[time] += power
                iterator_time = np.arange(0,end_time[-1],1)
                for time in iterator_time:
                    e_load[time] += power
...
def unmanaged_usecase_setup(useCase):
    ...
    if use_case == 2:
        ...
        avg_load, bottom_load, upper_load, avg_wait_fast = unmanaged_charging(num_fast_chrg_vehicles, power_v0g_fast, num_fast_chargers)
...
def v1g_optimize(num_v1g_vehicles):
    ...
    model = lp.LpProblem('Minimize the peak load', lp.LpMinimize)
    
    # define optimization varatibles
    ...
    for t in time_interval:
        model += max_load >= total_load[t]
        model += min_load <= total_load[t]
    for t in time_interval:
        model += lp.lpSum([charge_profiles[i,t]] for i in veh_v1g)+base_load[t]+unmanaged_load[t] == total_load[t]
    for veh in veh_v1g:
        if v1g_start_time[veh] >= v1g_end_time[veh]:
            for t in range(v1g_end_time[veh]):
                model += charge_profiles[veh,t] <= charge_states[veh,t]*power_v1g_upper_bound
                model += charge_profiles[veh,t] >= charge_states[veh,t]*power_v1g_lower_bound
            for t in range(v1g_end_time[veh],v1g_start_time[veh],1):
                model += charge_profiles[veh,t] == 0 
                model += charge_states[veh,t] == 0 
            for t in range(v1g_start_time[veh],24,1):
                model += charge_profiles[veh,t] <= charge_states[veh,t]*power_v1g_upper_bound
                model += charge_profiles[veh,t] >= charge_states[veh,t]*power_v1g_lower_bound
        if v1g_start_time[veh] < v1g_end_time[veh]:
            for t in range(v1g_start_time[veh]):
                model += charge_profiles[veh,t] == 0 
                model += charge_states[veh,t] == 0 
            for t in range(v1g_start_time[veh],v1g_end_time[veh],1):
                model += charge_profiles[veh,t] <= charge_states[veh,t]*power_v1g_upper_bound
                model += charge_profiles[veh,t] >= charge_states[veh,t]*power_v1g_lower_bound
            for t in range(v1g_end_time[veh],24,1):
                model += charge_profiles[veh,t]==0
                model += charge_states[veh,t]==0
                
    for veh in veh_v1g:
        temp_start = v1g_start_time[veh]
        temp_end = v1g_end_time[veh]
        temp_start_soc = v1g_start_SoC[veh]
        
        if temp_start > temp_end:
            for time in range(temp_start+1,24,1):
                temp_timer = range(temp_start,time)
                model += temp_start_soc + lp.lpSum( [charge_profiles[veh,t] * chrg_eff/bat_cap] for t in temp_timer) <= 1
            for time in range(0,temp_end+1):
                temp_timer = range(0,t,1)
                model += temp_start_soc + lp.lpSum([charge_profiles[veh,t] * chrg_eff/bat_cap] for t in range(temp_start,24,1)) 
                + lp.lpSum( [ charge_profiles[veh,t]*chrg_eff/bat_cap] for t in temp_timer) <= 1
                
            if end_SoC == 2:
                model += temp_start_soc + lp.lpSum([charge_profiles[veh,t] * chrg_eff/bat_cap] for t in range(temp_start,24,1)) 
                + lp.lpSum( [ charge_profiles[veh,t]*chrg_eff/bat_cap] for t in range(0,temp_end)) == 1
        ...
        if temp_start < temp_end:
            for t in range(temp_start+1, temp_end+1,1):
                ...
                model += temp_start_soc + lp.lpSum( [charge_profiles[veh,t] * chrg_eff/bat_cap] for t in temp_timer) <= 1
            if end_SoC == 2:
                model += temp_start_soc + lp.lpSum([charge_profiles[veh,t]*chrg_eff/bat_cap] for t in range(temp_start,temp_end,1)) == 1
    status = model.solve()
...
def loadAnalysis(charge_profiles, total_load):
    ...
    ax1.fill_between(x, optimized_ev_load, color='cyan', label='EV Load: Managed Charging')
...
if __name__ == '__main__':
    ...
    charging_strategy = 1 # 0 - unmanaged only, 1 - managed v1g (includes unmanaged), 2 - managed v2g(includes unmanaged)
    is_slow = 2 # slow charging
    
    
    # Initiate the number and enrollment of vehicles
    num_vehicles = np.int_(150) # Total number of EVs to be simulated
    if charging_strategy  == 0:
        ...
        percent_v1g = 0.2  # percentage of vehicles enrolled in V1G. Can take a value between 0 and 1
        ...
        if use_case == 2:
            ...
            power_v0g_slow = 0
            power_v0g_fast = 10 
            power_v2g_upper_bound = -7 
            power_v2g_lower_bound= -10 
    ...
    EV_load,optimized_total_load = loadAnalysis(charge_profiles, total_load)
